...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        double compute_duration = (end_time - start_time) * 1e-3;
        
        // #pragma omp critical
        // {
        //     { //if(ggml_op_to_string(node->op) == "GGML_OP_MUL_MAT")
        //         printf("=======================================\n");
        //         printf("%s\n", node->name);
        ...
        //             if (node->src[0]) {
        //             printf("src[0] name      : %s\n", node->src[0]->name);
        //             printf("src[0] rank      : %" PRId64 " \n", ggml_n_dims(node->src[0]));
        //             printf("src[0] shape     : [%lld, %lld, %lld, %lld]\n", node->src[0]->ne[3], node->src[0]->ne[2], node->src[0]->ne[1], node->src[0]->ne[0]);
        //             printf("src[0] type      : %s\n", ggml_type_to_string(node->src[0]->type));
        //         }

        //         if (node->src[1]) {
        //             printf("src[1] name      : %s\n", node->src[1]->name);
        //             printf("src[1] rank      : %" PRId64 " \n", ggml_n_dims(node->src[1]));
        //             printf("src[1] shape     : [%lld, %lld, %lld, %lld]\n", node->src[1]->ne[3], node->src[1]->ne[2], node->src[1]->ne[1], node->src[1]->ne[0]);
        ...
        //         printf("start_time: %" PRId64 " ms\n", start_time);
        //         printf("end_time: %" PRId64 " ms\n", end_time);
