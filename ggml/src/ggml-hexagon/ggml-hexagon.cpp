...
static void * ggmlhexagon_type_trait_tmp(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    ...
    GGML_TENSOR_BINARY_OP_LOCALS  // 텐서 차원, ne0~ne3, nb0~nb3 등 자동 정의

    GGML_ASSERT(nb10 == ggml_type_size(src1_type));

    const int64_t ne_plane = ne01 * ne00;

    // FP32 → FP16 변환 시 필요한 버퍼 크기 계산
    const size_t desired_size = (src1_type == GGML_TYPE_F32) ? ne03 * ne02 * ne_plane * sizeof(ggml_fp16_t) : 0;
    ctx->desired_size = desired_size;
    ...
    ctx->n_threads = std::thread::hardware_concurrency();
    ggml_fp16_t * wdata = (ggml_fp16_t *) ctx->work_data.get();

    if (src1_type == GGML_TYPE_F32) {
        const ggml_type_traits * traits = ggml_get_type_traits(src1_type);
        ggml_from_float_t from_float = traits->from_float_ref;

        for (int64_t i03 = 0; i03 < ne03; ++i03) {
            for (int64_t i02 = 0; i02 < ne02; ++i02) {
                const char * x_plane = (const char *)src1->data + i02 * nb12 + i03 * nb13;
                ggml_fp16_t * wplane = wdata + i02 * ne_plane + i03 * ne02 * ne_plane;
                ...
                for (int i = 1; i < n_threads; ++i) {
                    ...
                    if (start < end) {
                        ctx->tasks.push_back(std::async(std::launch::async, [=]() {
                            for (int64_t i01 = start; i01 < end; ++i01) {
                                const float * row_src = (const float *)(x_plane + i01 * nb11);
                                ggml_fp16_t * row_dst = wplane + i01 * ne00;
                                from_float(row_src, row_dst, ne00);
                            }
                        }));
                    }
                }
...
private:
    ...
    QnnLog_Level_t _qnn_log_level           = QNN_LOG_LEVEL_VERBOSE; //QNN_LOG_LEVEL_DEBUG

    qnn_profile_level _profile_level        = PROFILE_DETAIL; //PROFILE_DETAIL //PROFILE_OFF
...
static Qnn_Tensor_t * ggmlqnn_create_general_tensor_tmp(qnn_instance * instance, Qnn_GraphHandle_t graph_handle,
                                                    const ggml_tensor * tensor, const char * name,
                                                    Qnn_TensorType_t qnn_tensor_type,
                                                    Qnn_DataType_t qnn_data_type,
                                                    uint32_t rank, uint32_t * dims,
...
// Q8_0 텐서 생성 함수
struct ggml_tensor * create_q8_0_tensor_2048x64(struct ggml_context * ctx) {
    const int64_t ne[2] = { 4096, 14336 };  // [cols, rows] 64, 2048
    struct ggml_tensor * tensor = ggml_new_tensor(ctx, GGML_TYPE_F16, 2, ne);
    return tensor;
}

// Q8_0 데이터 채우기
void fill_q8_0_tensor(struct ggml_tensor * tensor) {
    const int64_t nrows = tensor->ne[1];  // 2048
    const int64_t ncols = tensor->ne[0];  // 64
    assert(ncols % 32 == 0);              // block 단위 확인

    const int blocks_per_row = ncols / 32;
    ggml_q8_0_block_t * data = (ggml_q8_0_block_t *) tensor->data;

    for (int64_t row = 0; row < nrows; ++row) {
        for (int b = 0; b < blocks_per_row; ++b) {
            ggml_q8_0_block_t * block = &data[row * blocks_per_row + b];
            block->d = 0.1f;  // 예시 scale 값
            for (int i = 0; i < 32; ++i) {
                block->qs[i] = (int8_t)((i + b + row) % 127 - 63); // 임의의 패턴값
            }
        }
    }
}

// 모든 Q8_0 값을 1로 채우는 디버깅용 함수
void fill_q8_0_tensor_all_ones(struct ggml_tensor * tensor, float scale = 0.1f) {
    const int64_t nrows = tensor->ne[1];
    const int64_t ncols = tensor->ne[0];
    assert(ncols % 32 == 0);  // Q8_0은 32단위 블록
    ...
    for (int64_t row = 0; row < nrows; ++row) {
        for (int b = 0; b < blocks_per_row; ++b) {
            ggml_q8_0_block_t * block = &data[row * blocks_per_row + b];
            block->d = scale; //scale
            float one_bytes = 0.0f;
            uint32_t pattern = 0x01000100;
            memcpy(&one_bytes, &pattern, sizeof(float));
            block->d = one_bytes;
            for (int i = 0; i < 32; ++i) {
                // if (i % 2 == 0) {
                //     block->qs[i] = 0;
...
void overwrite_tensor_raw_bytes(struct ggml_tensor *tensor) {
    // 1. 전체를 0으로 초기화
    ...
    memcpy(tensor->data, pattern, sizeof(pattern));
}

void overwrite_tensor_raw_bytes_1(struct ggml_tensor *tensor) {
    // 1. 전체를 0으로 초기화
    memset(tensor->data, 0, ggml_nbytes(tensor));

    // 2. 앞부분에 패턴 덮어쓰기 //_Float16
    float pattern[] = {
...
void overwrite_tensor_raw_bytes_2(struct ggml_tensor *tensor) {
    // 1. 전체를 0으로 초기화
    memset(tensor->data, 0, ggml_nbytes(tensor));
...
void init_tensor_raw_bytes(struct ggml_tensor *tensor) {
    // 1. 전체를 0으로 초기화
    memset(tensor->data, 0, ggml_nbytes(tensor));

}

void print_tensor_raw(const ggml_tensor *t, size_t count = 4) {
    std::cout << "Raw tensor : '" << t->name << "' (type = " << t->type << ")\n";

    switch (t->type) {
        case GGML_TYPE_F32: {
            auto *data = reinterpret_cast<float*>(t->data);
            printf("[");
            for (uint32_t i=0;i<count;i++) printf("%f%s", (float)data[i], (i+1==count?"]\n":", "));
            break;
        }
        case GGML_TYPE_F16: {
            auto *data = reinterpret_cast<_Float16*>(t->data);
            printf("[");
            for (uint32_t i=0;i<count;i++) printf("%f%s", (float)data[i], (i+1==count?"]\n":", "));
            break;
        }
        ...
        default:
            std::cout << "Tensor type " << t->type << " not supported for raw print.\n";
            break;
    }
}

void print_tensor_raw_tmp(const ggml_tensor *t, size_t count = 1024) {
    std::cout << "Raw tensor dump: '" << t->name << "' (type = " << t->type << ")\n";
    const uint8_t *data = static_cast<const uint8_t *>(t->data);
    for (size_t i = 0; i < count; ++i) {
        std::cout << "[" << i << "] = 0x"
                    << std::hex << std::setw(2) << std::setfill('0')
...
struct ggml_tensor * create_f16_tensor_src0(struct ggml_context * ctx) {
    const int64_t ne[2] = { 14336, 4096 };  // [cols, rows]
    return ggml_new_tensor(ctx, GGML_TYPE_F16, 2, ne);
}

struct ggml_tensor * create_f16_tensor_src1(struct ggml_context * ctx) {
    const int64_t ne[2] = { 25, 14336 };  // [cols, rows]
    return ggml_new_tensor(ctx, GGML_TYPE_F16, 2, ne);
}

struct ggml_tensor * create_f16_tensor_dst(struct ggml_context * ctx) {
    const int64_t ne[2] = { 4096, 25 };  // [cols, rows]
    return ggml_new_tensor(ctx, GGML_TYPE_F16, 2, ne);
}

struct ggml_tensor * create_f16_tensor_src0_4d(struct ggml_context * ctx) {
    const int64_t ne[4] = { 2, 6, 1, 1 };  // [cols, rows]
    return ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
}

struct ggml_tensor * create_f16_tensor_src1_4d(struct ggml_context * ctx) {
    const int64_t ne[4] = { 2, 2, 1, 1 };  // [cols, rows]
    return ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
}

struct ggml_tensor * create_f16_tensor_dst_4d(struct ggml_context * ctx) {
    const int64_t ne[4] = { 2, 6, 1, 1 };  // [cols, rows]
    return ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
}

static void * ggmlhexagon_flatdata(ggml_tensor * tensor, int8_t * out_data, uint16_t * out_scales) {
    GGML_ASSERT(tensor->type == GGML_TYPE_Q8_0);

    const int64_t nrow = tensor->ne[1];  // row 수
    const int64_t ncol = tensor->ne[0];  // col 수
    GGML_ASSERT(ncol % 32 == 0);         // Q8_0 구조 가정

    const char * src = (const char *) tensor->data;
    size_t nb0 = tensor->nb[0];  // 일반적으로 1
    size_t nb1 = tensor->nb[1];  // row 간 stride

    const int blocks_per_row = ncol / 32;
    const int block_size = 32;

    for (int64_t row = 0; row < nrow; ++row) {
        const char * row_ptr = src + row * nb1;

        // 데이터 블럭 (int8 값들)
        const int8_t * row_data = (const int8_t *)(row_ptr);
        memcpy(out_data + row * ncol, row_data, ncol);

        // 디버깅: int8 값 출력
        std::cout << "row " << row << " data: ";
        for (int64_t i = 0; i < ncol; ++i) {
            std::cout << std::setw(4) << static_cast<int>(row_data[i]);
        }
        std::cout << std::endl;

        // 스케일 값
        const uint16_t * scale_ptr = (const uint16_t *)(row_ptr + ncol);
        out_scales[row] = *scale_ptr;

        // 디버깅: 스케일 출력 (16진수 및 10진수)
        std::cout << "row " << row << " scale: 0x" 
                  << std::hex << std::setw(4) << std::setfill('0') << out_scales[row]
                  << std::dec << " (" << out_scales[row] << ")" << std::endl;
    }
}
...
static void ggmlqnn_compute_mul_mat_4d(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    ...
    GGMLQNN_CHECK_PARAMS(ctx, src0, src1, dst);
    GGML_ASSERT(ggml_n_dims_tmp(src0) == 4 && ggml_n_dims_tmp(src1) == 4); //rank4로 인해 변경
    ...
    GGMLHEXAGON_LOG_DEBUG("ggmlqnn_compute_mul_mat_4d_4d_4d");
...
static Qnn_Param_t makeBoolParam(const char* name, bool v) {
    Qnn_Param_t p{}; 
    p.paramType = QNN_PARAMTYPE_SCALAR;
    p.name = name;
    p.scalarParam.dataType = QNN_DATATYPE_BOOL_8;    // ← bool8 타입
    p.scalarParam.bool8Value = (uint8_t)(v ? 1 : 0); // ← QNN 버전에 따라 필드명이 boolValue/bool8Value일 수 있음
    return p;
}

static void ggmlqnn_compute_mul_mat_4d_tmp(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    Qnn_ErrorHandle_t error     = QNN_SUCCESS;
    qnn_instance * instance     = ctx->instance;
    QNN_INTERFACE_VER_TYPE qnn_raw_interface = ctx->raw_interface;

    // struct ggml_init_params params = {
    //     .mem_size   = 128*1024*1024, // 충분한 버퍼
    //     .mem_buffer = malloc(128*1024*1024),
    //     .no_alloc   = false,
    // };
    // struct ggml_context * ctx_1 = ggml_init(params);

    // struct ggml_tensor * q8_tensor = create_f16_tensor_src0_4d(ctx_1);
    // struct ggml_tensor * q8_tensor_1 = create_f16_tensor_src1_4d(ctx_1);
    // struct ggml_tensor * q8_tensor_2 = create_f16_tensor_dst_4d(ctx_1);
    // overwrite_tensor_raw_bytes_1(q8_tensor);
    // overwrite_tensor_raw_bytes_2(q8_tensor_1);

    // const ggml_tensor * src0 = q8_tensor;
    // const ggml_tensor * src1 = q8_tensor_1;
    // ggml_tensor * dst        = q8_tensor_2;

    const ggml_tensor * src0 = op->src[1];
    const ggml_tensor * src1 = op->src[0];
    ggml_tensor * dst        = op;
    ...
    std::string graph_name;
    ggmlhexagon_get_opkey_from_op(op, graph_name);
    graph_name = graph_name + "_4d";
    GGMLHEXAGON_LOG_DEBUG("graph name %s\n", graph_name.c_str());
    GGMLHEXAGON_LOG_DEBUG("ggmlqnn_compute_mul_mat_4d_tmp");
    ...
    Qnn_Tensor_t * p_matmul_out     = nullptr;
    Qnn_Tensor_t * p_reshape2_out   = nullptr;
    std::cout<<"now_time_3 : "<<ggml_time_us()<<" ms"<<std::endl;
    if (ctx->qnn_singlenode_graph_map.find(graph_name) != ctx->qnn_singlenode_graph_map.end()) {
        qnn_singlenode_res_t & graph_item   = ctx->qnn_singlenode_graph_map[graph_name];
        graph_handle                        = std::get<0>(graph_item);
        qnn_ptensors_t & tensors            = std::get<1>(graph_item);
        ...
        std::cout << "src0.ne = [" << (long long)src0->ne[3] << ", "
                                << (long long)src0->ne[2] << ", "
                                << (long long)src0->ne[1] << ", "
                                << (long long)src0->ne[0] << "]\n";
        std::cout << "src1.ne = [" << (long long)src1->ne[3] << ", "
                                << (long long)src1->ne[2] << ", "
                                << (long long)src1->ne[1] << ", "
                                << (long long)src1->ne[0] << "]\n";
        std::cout << "dst.ne  = [" << (long long)dst->ne[3]  << ", "
                                << (long long)dst->ne[2]  << ", "
                                << (long long)dst->ne[1]  << ", "
                                << (long long)dst->ne[0]  << "]\n";
    ...
    if (src1_type != GGML_TYPE_F32) {
        ...
        QNN_VER_PTR(*p_tensor1)->clientBuf = {wdata, static_cast<uint32_t>(desired_size)};
    } else {
        std::cout<<"not came here"<<std::endl;
        QNN_VER_PTR(*p_tensor1)->clientBuf = {src1->data, static_cast<uint32_t>(ggml_nbytes(src1))};
    }
    //QNN_VER_PTR(*p_tensor1)->clientBuf = {src1->data, static_cast<uint32_t>(ggml_nbytes(src1))};
    QNN_VER_PTR(*p_reshape2_out)->clientBuf = {dst->data, static_cast<uint32_t>(ggml_nbytes(dst))};

    std::cout<<"src0 size : "<<ggmlqnn_get_tensor_data_size(src0)<<std::endl;
    std::cout<<"src1 size : "<<ggmlqnn_get_tensor_data_size(src1)<<std::endl;
    std::cout<<"src1_wdata size : "<<static_cast<uint32_t>(desired_size)<<std::endl;
    std::cout<<"dstt size : "<<ggmlqnn_get_tensor_data_size(dst)<<std::endl;
    Qnn_Tensor_t input_tensors[]    = {*p_tensor0, *p_tensor1};
    Qnn_Tensor_t output_tensors[]   = {*p_reshape2_out};
    print_tensor_raw(src0, 12);
    print_tensor_raw(src1, 4);
    std::cout<<"now_time_4 : "<<ggml_time_us()<<" ms"<<std::endl;
    CHECK_QNN_API(error, qnn_raw_interface.graphExecute(graph_handle, input_tensors, 2, output_tensors, 1, NULL, NULL));
    std::cout<<"now_time_5 : "<<ggml_time_us()<<" ms"<<std::endl;
    print_tensor_raw(dst, 12);
    op_perf.info();
}
...
static void ggmlqnn_compute_mul_mat(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    ...
    const enum ggml_type src0_type              = src0->type;
    const uint32_t src0_rank                    = ggml_n_dims_tmp(src0);
    const uint32_t src1_rank                    = ggml_n_dims_tmp(src1);
    const char * ggml_original_opname           = ggml_op_name(op->op);
    ggmlhexagon_print_tensors_info(__func__, ctx, src0, src1, dst);
    ...
    GGMLHEXAGON_LOG_DEBUG("src0_rank % " PRIu32 ", src1_rank % " PRIu32 "", src0_rank, src1_rank);
    if (4 == src0_rank) {
        GGMLHEXAGON_LOG_DEBUG("go_4d_tmp");
        return ggmlqnn_compute_mul_mat_4d_tmp(ctx, op);
    }

    void * wdata                                = ggmlhexagon_type_trait(ctx, op);
    const size_t desired_size                   = ctx->desired_size;
    double start_time = ggml_time_us();
    if (ctx->qnn_singlenode_graph_map.find(graph_name) != ctx->qnn_singlenode_graph_map.end()) {
        //retrieve computational resource from cached QNN graph
        qnn_singlenode_res_t & graph_item = ctx->qnn_singlenode_graph_map[graph_name];
        graph_handle = std::get<0>(graph_item);
    ...
    CHECK_QNN_API(error, qnn_raw_interface.graphExecute(graph_handle,
                                                        ...
                                                        tensor_outputs, 1,
                                                        nullptr, nullptr));
    double end_time = ggml_time_us();
    double duration_time = (end_time - start_time) * 1e-3;
    std::cout<<"start_time : "<<start_time<<" ms"<<std::endl;
    std::cout<<"end_time : "<<end_time<<" ms"<<std::endl;
    std::cout<<"graph_duration_time : "<<duration_time<<" ms"<<std::endl;
    op_perf.info();
}

static void ggmlqnn_compute_mul_mat_FP32(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    Qnn_ErrorHandle_t error                     = QNN_SUCCESS;
    qnn_instance * instance                     = nullptr;
    Qnn_GraphHandle_t graph_handle              = nullptr;
    Qnn_Tensor_t * p_tensor0                    = nullptr;
...
static void ggmlqnn_compute_mul_mat_FP16(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    Qnn_ErrorHandle_t error                     = QNN_SUCCESS;
    qnn_instance * instance                     = nullptr;
    Qnn_GraphHandle_t graph_handle              = nullptr;
    Qnn_Tensor_t * p_tensor0                    = nullptr;
    ...
    if (ctx->qnn_singlenode_graph_map.find(graph_name) != ctx->qnn_singlenode_graph_map.end()) {
        ...
        p_tensor0 = ggmlqnn_create_general_tensor(instance, graph_handle, src0, nullptr,
                                                  QNN_TENSOR_TYPE_APP_WRITE,
                                                  QNN_DATATYPE_FLOAT_16, src0_rank,
                                                  nullptr, nullptr, 0);
        p_tensor1 = ggmlqnn_create_general_tensor(instance, graph_handle, src1, nullptr,
                                                  QNN_TENSOR_TYPE_APP_WRITE,
                                                  QNN_DATATYPE_FLOAT_16, src0_rank,
...
static void ggmlqnn_compute_mul_mat_int(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    Qnn_ErrorHandle_t error                     = QNN_SUCCESS;
    qnn_instance * instance                     = nullptr;
    Qnn_GraphHandle_t graph_handle              = nullptr;
    Qnn_Tensor_t * p_tensor0                    = nullptr;
    ...
    print_tensor_raw(src0, 6);
    print_tensor_raw(src1, 6);
    CHECK_QNN_API(error, qnn_raw_interface.graphExecute(graph_handle,
                                                        tensor_inputs, 2,
                                                        tensor_outputs, 1,
                                                        nullptr, nullptr));
    print_tensor_raw(dst, 4);
    double end_time = ggml_time_us();
    double duration_time = (end_time - start_time) * 1e-3;
    std::cout<<"start_time : "<<start_time<<" ms"<<std::endl;
    std::cout<<"end_time : "<<end_time<<" ms"<<std::endl;
...
static void ggmlqnn_compute_mul_mat_tmp(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    struct ggml_init_params params = {
    .mem_size   = 128*1024*1024, // 충분한 버퍼
    .mem_buffer = malloc(128*1024*1024),
    .no_alloc   = false,
    ...
    struct ggml_tensor * q8_tensor = create_f16_tensor_src0(ctx_1);
    struct ggml_tensor * q8_tensor_1 = create_f16_tensor_src1(ctx_1);
    struct ggml_tensor * q8_tensor_2 = create_f16_tensor_dst(ctx_1);
    overwrite_tensor_raw_bytes_1(q8_tensor);
    overwrite_tensor_raw_bytes_2(q8_tensor_1);
    //fill_q8_0_tensor(q8_tensor);
    // init_tensor_raw_bytes(q8_tensor_2);
    ...
    Qnn_Tensor_t * p_param_tensor               = nullptr;
    Qnn_Tensor_t * p_tensor2_transpose          = nullptr;
    const ggml_tensor * src0                    = q8_tensor; //op->src[0]
    const ggml_tensor * src1                    = q8_tensor_1; //q8_tensor
    ggml_tensor       * dst                     = q8_tensor_2; //op
    ...
    // int8_t * out_data;
    // uint16_t * out_scales;
    // void * flatdata = ggmlhexagon_flatdata(op->src[0], out_data, out_scales);
    ...
    if (ctx->qnn_singlenode_graph_map.find(graph_name) != ctx->qnn_singlenode_graph_map.end()) {
        ...
        //create QNN graph
        std::cout<<"now_time_1-1 : "<<ggml_time_us()<<" ms"<<std::endl;
        std::cout<<"now_time_1_0 : "<<ggml_time_us()<<" ms"<<std::endl;
        GGMLHEXAGON_LOG_VERBOSE("graph name %s", graph_name.c_str());
        error = instance->init_qnn_graph(graph_name, static_cast<HEXAGONBackend>(ctx->device),
                                         g_hexagon_appcfg.vtcm_size_in_mb,
                                         g_hexagon_appcfg.hvx_threads);
        ...
        std::cout<<"now_time_1_1 : "<<ggml_time_us()<<" ms"<<std::endl;
        graph_handle = instance->get_qnn_graph_handle();
        std::cout<<"now_time_1_2 : "<<ggml_time_us()<<" ms"<<std::endl;
        //create computational tensor
        p_tensor0 = ggmlqnn_create_general_tensor_tmp(instance, graph_handle, src0, nullptr,
                                                  QNN_TENSOR_TYPE_APP_WRITE,
                                                  QNN_DATATYPE_FLOAT_16, src0_rank,
                                                  nullptr, nullptr, 0);
        std::cout<<"now_time_1_3 : "<<ggml_time_us()<<" ms"<<std::endl;
        p_tensor1 = ggmlqnn_create_general_tensor_tmp(instance, graph_handle, src1, nullptr,
                                                  QNN_TENSOR_TYPE_APP_WRITE,
                                                  QNN_DATATYPE_FLOAT_16, src0_rank,
                                                  nullptr, nullptr, 0);
        std::cout<<"now_time_1_4 : "<<ggml_time_us()<<" ms"<<std::endl;
        p_tensor2 = ggmlqnn_create_general_tensor_tmp(instance, graph_handle, dst, nullptr,
                                                  QNN_TENSOR_TYPE_APP_READ,
                                                  QNN_DATATYPE_FLOAT_16, src0_rank,
                                                  nullptr, nullptr, 0);
        std::cout<<"now_time_2_0 : "<<ggml_time_us()<<" ms"<<std::endl;
        //create param tensor for offload 2d/3d/4d matrix multiplication
        // const uint32_t param_tensor_data[GGML_MAX_DIMS][GGML_MAX_DIMS] = {
        //         {0},
        //         {1, 0},
        ...
        //                                                     QNN_DATATYPE_FLOAT_16, src0_rank,
        //                                                     nullptr, nullptr, 0, true);
        std::cout<<"now_time_3_0 : "<<ggml_time_us()<<" ms"<<std::endl;
        //compose QNN graph: add mulmat node
        Qnn_Param_t out_0_params[] = {
                {.paramType = QNN_PARAMTYPE_SCALAR, .name = QNN_OP_MAT_MUL_PARAM_TRANSPOSE_IN1, .scalarParam = {
                        .dataType = QNN_DATATYPE_BOOL_8, .bool8Value = 0}}};
        Qnn_Tensor_t out_0_inputs[] = {*p_tensor0, *p_tensor1};
        Qnn_Tensor_t out_0_outputs[] = {*p_tensor2};
        Qnn_OpConfig_t out_0 = ggmlqnn_create_op_config("mulmat_opconfig",
                                                        QNN_OP_PACKAGE_NAME_QTI_AISW,
                                                        QNN_OP_MAT_MUL, out_0_params, 1,
                                                        out_0_inputs, 2, out_0_outputs, 1);
        CHECK_QNN_API(error, qnn_raw_interface.graphAddNode(graph_handle, out_0));
        std::cout<<"now_time_4_0 : "<<ggml_time_us()<<" ms"<<std::endl;
        //compose QNN graph: add transpose node
        // Qnn_Param_t out_trans1_0_params[] = {
        //         {.paramType = QNN_PARAMTYPE_TENSOR, .name = "perm", .tensorParam = *p_param_tensor}};
        // Qnn_Tensor_t out_trans1_0_inputs[] = {*p_tensor2_transpose};
        // Qnn_Tensor_t out_trans1_0_outputs[] = {*p_tensor2};
        // std::cout<<"now_time_4_1 : "<<ggml_time_us()<<" ms"<<std::endl;
        // Qnn_OpConfig_t out_trans1_0 = ggmlqnn_create_op_config("mulmat_transpose_opconfig",
        //                                                        QNN_OP_PACKAGE_NAME_QTI_AISW,
        //                                                        QNN_OP_TRANSPOSE,
        //                                                        out_trans1_0_params, 1,
        //                                                        out_trans1_0_inputs, 1,
        //                                                        out_trans1_0_outputs, 1);
        // std::cout<<"now_time_4_2 : "<<ggml_time_us()<<" ms"<<std::endl;
        // CHECK_QNN_API(error, qnn_raw_interface.graphAddNode(graph_handle, out_trans1_0));
        // std::cout<<"now_time_4_3 : "<<ggml_time_us()<<" ms"<<std::endl;
        //finalize QNN graph
        CHECK_QNN_API(error, qnn_raw_interface.graphFinalize(graph_handle, nullptr, nullptr));
        std::cout<<"now_time_5_0 : "<<ggml_time_us()<<" ms"<<std::endl;
        //cache QNN graph
        qnn_ptensors_t ggml_op_mulmat_tensors;
        ggml_op_mulmat_tensors.reserve(3);
        ggml_op_mulmat_tensors.push_back(p_tensor0);
        ggml_op_mulmat_tensors.push_back(p_tensor1);
        ggml_op_mulmat_tensors.push_back(p_tensor2);
        //ggml_op_mulmat_tensors.push_back(p_param_tensor);
        ...
        auto graph_item = std::make_tuple(graph_handle, ggml_op_mulmat_tensors);
        ctx->qnn_singlenode_graph_map[graph_name] = graph_item;
        std::cout<<"now_time_6_0 : "<<ggml_time_us()<<" ms"<<std::endl;
    }
        ...
        case GGML_OP_MUL_MAT:
        {
            ...
            if (ctx->device == HEXAGON_BACKEND_QNNNPU) {
                if (1 == g_hexagon_appcfg.enable_q_mulmat) {
                    return (src0->type == GGML_TYPE_F32
                        || src0->type == GGML_TYPE_Q4_0 || src0->type == GGML_TYPE_Q8_0
                        || src0->type == GGML_TYPE_Q6_K || src0->type == GGML_TYPE_Q8_K || src0->type == GGML_TYPE_F16
                        ) && (src1->type == GGML_TYPE_F32) && (op_tensor->type == GGML_TYPE_F32);
                } else {
...
static bool ggmlhexagon_compute_forward(ggml_backend_t backend, struct ggml_tensor * dst) {
    ...
    switch (dst->op) {
        ...
        case GGML_OP_MUL_MAT:
            ...
            //ggmlqnn_compute_mul_mat_tmp(ctx, dst);
            //ggmlqnn_compute_mul_mat_int(ctx, dst);
            //ggmlqnn_compute_mul_mat_FP32(ctx, dst);
            //ggmlqnn_compute_mul_mat_FP16(ctx, dst);
            break;
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT) {
            ...
            if (node->src[0]) {
                ...
                std::cout << "src[0] rank : " << ggml_n_dims(node->src[0]) << std::endl;
                std::cout << "src[0] shape: [" << node->src[0]->ne[3] << ", " << node->src[0]->ne[2] << ", " << node->src[0]->ne[1] << ", " << node->src[0]->ne[0] << "]" << std::endl;
                std::cout << "src[0] type : " << ggml_type_to_string(node->src[0]->type) << std::endl;
            }

            if (node->src[1]) {
                std::cout << "src[1] name : " << node->src[1]->name << std::endl;
                std::cout << "src[1] rank : " << ggml_n_dims(node->src[1]) << std::endl;
                std::cout << "src[1] shape: [" << node->src[1]->ne[3] << ", " << node->src[1]->ne[2] << ", " << node->src[1]->ne[1] << ", " << node->src[1]->ne[0] << "]" << std::endl;
                std::cout << "src[1] type : " << ggml_type_to_string(node->src[1]->type) << std::endl;
            }
            std::cout<<"start_time : "<<start_time / 1000<<" ms"<<std::endl;
            std::cout<<"end_time : "<<end_time / 1000<<" ms"<<std::endl;
